...
Dati i sistemi di calcolo $\C_1$ e $\C_2$, una traduzione da $\C_1$ a $\C_2$ è una funzione
$$ T:\C_1\text{-PROG}\to\C_2\text{-PROG} $$
tale che $T$ sia:
\begin{enumerate}
    \item Programmabile: è programmabile effettivamente
    \item Completa: traduce ogni programma
    \item Corretta: mantiene la semantica:
        $$\forall P\in\C_1\text{-PROG} \quad \semanticaRAM_{T(P)}=\semanticaWHILE_{P}$$
\end{enumerate}
\subsubsection{Confronto tra F(RAM) e F(WHILE)}
Si prendano due sistemi di calcolo $\C_1$ e $\C_2$ con le rispettive potenze computazionali:
$$ F(\C_1) = \{\semanticaRAM_{P_1}:P_1\in\C_1\text{-PROG}\} $$
$$ F(\C_2) = \{\semanticaWHILE_{P_2}:P_2\in\C_2\text{-PROG}\} $$

Si vuole mostrare che $F(\C_1)\subseteq F(\C_2)$, ovvero che $\C_1$ non è più potente di $\C_2$;
per farlo bisogna dimostrare che:
\begin{table}[H]
    \centering
    \begin{tabular}{c}
        $\forall f \in F(\C_1) \quad f\in F(\C_2)$\\[1em]
        \rotatebox[origin=c]{90}{$\equiv$}\\[1em]
        $\exists P_1\in\C_1\text{-PROG}:f=\semanticaRAM_{P_1}
    \ \Rightarrow \ \exists P_2\in\C_2\text{-PROG}:f=\semanticaWHILE_{P_2}$
    \end{tabular}
\end{table}
In altre parole, per ogni programma di $\C_1$ ne esiste uno equivalente in $\C_2$.
\\
\begin{theorem}
    Se esiste una traduzione $T$ da $\C_1$ a $\C_2$, allora $F(\C_1)\subseteq F(\C_2)$.
\end{theorem}
\begin{proof}
    Si prenda una funzione $f$ tale che:
    $$ f\in F(\C_1) $$
    Si ha quindi che esiste un programma in $\C_1\text{-PROG}$ che può calcolare $f$:
    \begin{equation} \label{eq:traduzione_proof1}
        \exists P\in\C_1\text{-PROG}:f=\semanticaWHILE_P
    \end{equation}
    Si prenda la traduzione $T$ da $\C_1$ a $\C_2$; vista la completezza di $T$ si ha che:
    ...
    \end{equation}
    Si ha quindi che esiste un programma $T(P)$ in $\C_2$-PROG (\ref{eq:traduzione_proof2})
